...
以下に論文の本文（PDFから抽出したMarkdown）が与えられています。
**アブストラクトだけでなく、必ず本文全体を精読したうえで**要約を作成してください。
本文に記載されている具体的な手法の詳細、実験条件、数値結果、図表の内容に言及すること。
...
WHAT: どんなもの？ 3〜5文で。研究の目的・対象・成果物を説明。本文から具体的な技術名やシステム名を引用すること。
NOVEL: 先行研究と比べてどこがすごいか？ 箇条書き2〜4項目。本文中のRelated WorkやIntroductionで著者が挙げている差別化ポイントを正確に反映すること。
METHOD: 技術や手法のキモはどこ？ 4〜6文で核となるアプローチ・アーキテクチャ・アルゴリズムを説明。本文のMethodセクションに基づき、具体的なステップやパラメータに言及すること。
EVAL: どうやって有効だと検証した？ 3〜4文。本文のEvaluation/Resultsセクションに基づき、実験デザイン（参加者数、条件、ベースライン）と主要な定量結果（数値やp値）を含めること。
DISCUSSION: 議論・Limitation を2〜3文。著者自身が述べている限界や今後の課題を反映すること。
NEXT: 次に読むべき関連論文を2〜3件（タイトルと理由）。本文の参考文献リストから実際に引用されている論文を選ぶこと。

## 重要なルール
- **全セクションを日本語で書くこと（英語禁止）**
- ラベル（CLAIM:, WHAT: 等）は半角英字のまま使い、コロンの後に内容を書く
- 論文のタイトル・著者名・固有名詞はそのまま英語で書いてよい
- 各セクション間に空行を入れないこと
- **アブストラクトの言い換えではなく、本文の内容に基づいた深い要約を行うこと**
